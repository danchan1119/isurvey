...
<cfcomponent displayName="Question" hint="Handles all question interactions." output="false">
		...
		<cfloop index="x" from="1" to="#arrayLen(arguments.answers)#">
			<cfif structKeyExists(arguments.answers[x],"answer") and
				...
				<cfquery datasource="#variables.dsn#">
					insert into #variables.tableprefix#answers(id, questionidfk, answer, rank, code)
					...
					<cfqueryparam value="#arguments.answers[x].code#" cfsqltype="CF_SQL_INTEGER">)
				</cfquery>
			</cfif>
		</cfloop>
	...
	<cffunction name="addQuestion" access="public" returnType="string" output="false"
		...
		<cfargument name="header" type="string" required="false" hint="Section header.">
		...
		<cfquery datasource="#variables.dsn#">
			insert into #variables.tableprefix#questions(id, question, rank, required, surveyidfk, questiontypeidfk, header)
			values(
				<cfqueryparam value="#newid#" cfsqltype="CF_SQL_VARCHAR" maxlength="35">,
				<cfqueryparam value="#arguments.question#" cfsqltype="CF_SQL_VARCHAR" maxlength="500">,
				...
				<cfif isDefined("arguments.header")>
					<cfqueryparam value="#arguments.header#" cfsqltype="CF_SQL_VARCHAR" maxlength="255">
				<cfelse>
					<cfqueryparam value="#arguments.header#" cfsqltype="CF_SQL_VARCHAR" maxlength="255" null="true">
				</cfif>
	...
	<cffunction name="duplicateQuestion" access="public" returnType="void" output="false"
		...
		<cfloop query="answers">
			...
			<cfset aData[arrayLen(aData)].code = answers.code>			
		</cfloop>

		<cfset newq = addQuestion(q.question,q.rank,q.required,arguments.surveyidfk,q.questionTypeIDFK,q.header,aData)>
	...
	<cffunction name="getSkips" access="public" returnType="query" output="false"
				hint="Grabs entry criteria for a question.">
		<cfargument name="id" type="uuid" required="true" hint="The UUID for the question.">
		
		<cfset var q = "">
		
		<cfquery name="q" datasource="#variables.dsn#">
			select	id, criteriaquestion, criteriaquestionvalue, rank, nextquestion
			from	#variables.tableprefix#questionskips
			where	questionidfk = <cfqueryparam value="#arguments.id#" cfsqltype="CF_SQL_VARCHAR" maxlength="35">
			order by rank asc
		</cfquery>
	...
	<cffunction name="getQuestion" access="public" returnType="struct" output="false"
		...
		<cfquery name="qQuestion" datasource="#variables.dsn#">
			select	#variables.tableprefix#questions.id, surveyidfk, question, questiontypeidfk, rank, required, header,
		...
		<cfset result.skips = getSkips(arguments.id)>
	...
	<cffunction name="getQuestions" access="public" returnType="query" output="false"
		...
		<cfif isDefined("arguments.rank")>
			...
			<cfset queryAddColumn(qGetQuestions,"skips",arrayNew(1))>
			<cfset querySetCell(qGetQuestions,"skips",getSkips(qGetQuestions.id),1)>
		</cfif>
	...
	<cffunction name="updateAnswers" access="public" returnType="void" output="false"
		...
		<cfloop index="x" from="1" to="#arrayLen(arguments.answers)#">
			<cfif structKeyExists(arguments.answers[x], "answer") and
				  ...
				  structKeyExists(arguments.answers[x], "code") and
				...
				set
					...
					code = <cfqueryparam value="#arguments.answers[x].code#" cfsqltype="CF_SQL_INTEGER">
	...
	<cffunction name="updateQuestion" access="public" returnType="void" output="false"
		...
		<cfquery datasource="#variables.dsn#">
			update #variables.tableprefix#questions
				set
					question = <cfqueryparam value="#arguments.question#" cfsqltype="CF_SQL_VARCHAR" maxlength="500">,
					...
					<cfif isDefined("arguments.header")>
						,header = <cfqueryparam value="#arguments.header#" cfsqltype="CF_SQL_VARCHAR" maxlength="255">
					</cfif>
				where id = <cfqueryparam value="#arguments.ID#" cfsqltype="CF_SQL_VARCHAR" maxlength="35">
		</cfquery>
