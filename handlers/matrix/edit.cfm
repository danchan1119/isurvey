...
<!---
	...
	Last Updated : 
	History      : Added option for section header
	Purpose		 : 
--->
...
<cfif isDefined("attributes.question")>
	...
	<cfloop query="qitems">
		...
		<cfset items[arrayLen(items)].code = -1 * code>
...
<cfparam name="form.itemcode_new" default="">
<cfparam name="form.item_new2" default="">
<cfparam name="form.itemrank_new2" default="">
<cfparam name="form.itemcode_new2" default="">
...
<cfif isDefined("form.save") OR isDefined("form.saveaddmore")>
	...
	<cfif len(form.item_new)>
		...
		<cfelseif not len(form.itemcode_new) or not isNumeric(form.itemcode_new)>
			<cfset errors = errors & "Your code for the new item must be numeric.<br>">
		<cfelse>
			<cfset items[arrayLen(items)+1] = structNew()>
			<cfset items[arrayLen(items)].answer = form.item_new>
			<cfset items[arrayLen(items)].rank = form.itemrank_new>
			<cfset items[arrayLen(items)].code = form.itemcode_new>
			<cfset form.item_new = "">
	...
	<cfif len(form.item_new2)>
		<cfif not len(form.itemrank_new2) or not isNumeric(form.itemrank_new2) or form.itemrank_new2 lte 0>
			<cfset errors = errors & "Your rank for the new answer must be numeric and greater than one.<br>">
		<cfelseif not len(form.itemcode_new2) or not isNumeric(form.itemcode_new2)>
			<cfset errors = errors & "Your code for the new item must be numeric.<br>">
		<cfelse>
			<cfset items[arrayLen(items)+1] = structNew()>
			<cfset items[arrayLen(items)].answer = form.item_new2>
			<cfset items[arrayLen(items)].rank = form.itemrank_new2>
			<cfset items[arrayLen(items)].code = form.itemcode_new2>
	...
	<cfif not len(errors)>
		<cftry>
			...
			<cfloop index="x" from="1" to="#arrayLen(items)#">
				...
				<cfset tempItem.code = -1 * tempItem.code>
...
<table cellspacing=0 cellpadding=5 class="adminEditTable" width="100%">
	...
	<cfloop index="x" from="1" to="#arrayLen(items)#">
		...
		<tr valign="top">
			<td><b>Rank #x#:</b></td>
			<td><input type="text" name="itemrank#x#" value="#items[x].rank#" size="2"></td>
		</tr>
		<tr valign="top">
			<td><b>Code #x#:</b></td>
			<td><input type="text" name="itemcode#x#" value="#items[x].code#" size="2"> <cfif structKeyExists(items[x],"id")><input type="submit" name="delete_item" value="Delete Item #x#" onClick="return confirm('This change cannot be undone. Are you sure?')"></cfif></td>
		</tr>
	</cfloop>
	<tr>
		<td><b>New Item:</b></td>
	...
	<tr valign="top">
		<td><b>New Code:</b></td>
		<td><input type="text" name="itemcode_new" value="#form.itemcode_new#" size="2"></td>
	</tr>	
	<tr>
		<td><b>New Item:</b></td>
	...
	<tr valign="top">
		<td><b>New Code:</b></td>
		<td><input type="text" name="itemcode_new2" value="#form.itemcode_new2#" size="2"></td>
	</tr>	
